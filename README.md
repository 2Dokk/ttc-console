...
![지상국 관제 콘솔: ISS가 대전 지상국 상공을 지나는 패스 중. 업링크 손실 35% 환경에서 Go-Back-N 재전송 후 명령이 순서대로 실행되고, 교신 불가 구간의 텔레메트리가 레코더 재생(보라색)으로 채워진 모습](docs/console.png)
...
### 2. 시뮬레이션 시계
ISS가 대전 상공을 지나는 건 하루 7번 안팎, 한 번에 2~9분입니다. 실제 시간 그대로면 시연이 불가능하고 테스트도 쓸 수 없습니다.
